...
def get_weather(session_key: int, n_points: int = 57,
    ...
    p   = SESSION_PARAMS.get(_sk)  # None si session_key inconnu

    if p is not None:
        # GP connu dans le catalogue → données réalistes
        air_base = p["weather_temp"]
        trk_base = p["track_temp"]
        wnd_base = p["wind_speed"]
        _seed    = _sk % 99991
    else:
        # GP inconnu ou 2020-2022 → températures calculées sur year/round_num
        # Simulation réaliste : températures croissantes au fil de la saison
    ...
    try:
        ...
        # Approximation : date basée sur year et round_num
